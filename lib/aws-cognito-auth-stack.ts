...
export class AwsCognitoAuthStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const userPool: cognito.UserPool = new cognito.UserPool(this, 'kaamyabi-dev-pool', {
    ...
    const stack = new cdk.Stack(app, 'kaamyabi-auth-stack-test', { stackName: 'kaamyabi-auth-stack', env: { account: '320542485006', region: 'eu-west-1' } })

    //const stack = new Stack(app, 'kaamyabi-auth-stack-dev');

    const stackProps = { env: { region: "eu-west-1", account: "320542485006" } };

    //const awesomePool = cognito.UserPool.fromUserPoolId(stack, 'kaamyabi-dev-pool', 'eu-central-1');
    const awsCognitoTestStack = new AwsCognitoAuthStack(app, stack.stackName, stackProps);
    awsCognitoTestStack.templateOptions.transforms = ["AWS::Serverless-2016-10-31"];
    //app.synth();
  }
}
